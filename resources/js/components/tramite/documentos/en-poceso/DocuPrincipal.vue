...
<div>
    <form action="" @submit.prevent="guardarDocumento()">
        <div :class="'panel ' + (viewer ? 'panel-info' : 'panel-primary')">
            <div class="panel-body">
                <div class="form-group" style="width: 100%">
                    <div class="col-sm-12">
                        ...
                        <div v-else class="col-sm-4">
                            <label class="radio-inline">
                                <input v-model="formData.docu_origen" type="radio" value="1" class="radio-inline" @change="chngOrigen(1)" />
                                Interno
                            </label>
                            <label v-if="!plantilla" class="radio-inline">
                                <input v-model="formData.docu_origen" type="radio" value="2" class="radio-inline" @change="chngOrigen(2)" />
                                Externo
                            </label>
                        </div>
                    ...
                    <!--MOSTRAR OCULTO-->
                    <div v-show="parseInt(formData.docu_origen) === 2" :class="{ 'col-sm-12': true, 'has-error': errors.has('entidad') }">
                        ...
                        <div class="col-sm-6">
                            <select v-model="formData.docu_iddependencia" class="form-control">
                                <option v-for="(unidad, indexEntidad) in getEntidadesExternas" :key="indexEntidad" :value="unidad.iddependencia">
                                    {{ unidad.depe_nombre }}
                                </option>
                            </select>
                    ...
                    <div v-show="parseInt(formData.docu_origen) === 2" class="col-sm-12">
                        ...
                        <div class="col-sm-7">
                            <!-- dependencia-->
                            <input ref="dni" v-model="formData.docu_dni" type="text" class="form-control" name="dni" placeholder="DNI" @change="validarDNI" />
                        </div>
                    </div>
                    <div v-show="parseInt(formData.docu_origen) === 2" class="col-sm-12">
                        ...
                        <div :class="{ 'col-sm-7': true, 'has-error': errors.has('detalle') }">
                            <!-- dependencia-->
                            <input ref="detalle" v-model="formData.docu_detalle" v-validate="'max:60'" type="text" name="detalle" class="form-control" @change="formData.docu_detalle = formData.docu_detalle.toUpperCase()" />
                            <span v-show="errors.has('detalle')" class="help-block">{{ errors.first('detalle') }}</span>
                        </div>
                        <div class="col-sm-3">
                            <input ref="ruc" v-model="formData.docu_ruc" type="text" class="form-control" name="ruc" placeholder="RUC" @keyup="validarRUC" />
                        </div>
                    </div>
                    <!--FIN MOSTRAR OCULTO-->
                    <div v-if="parseInt(formData.docu_origen) === 1 && ((viewer && formData.docu_tipo) || !viewer)" class="col-sm-12">
                        ...
                        <div v-else class="col-sm-7">
                            <label class="checkbox-inline">
                                <input v-model="formData.docu_tipo" type="checkbox" name="docu_tipo" :disabled="edit" @change="chngTipo()" />Documento Personal</label>
                        </div>
                    </div>
                    ...
                    <div :class="{ 'col-sm-12': true, 'has-error': errors.has('firma') }">
                        ...
                        <div class="col-sm-7">
                            ...
                            <input v-show="parseInt(formData.docu_origen) !== 1" ref="firma" v-model="formData.docu_firma" v-validate="'required|max:60'" type="text" name="firma" class="form-control" @change="formData.docu_firma = formData.docu_firma.toUpperCase()" />
                    ...
                    <div :class="{ 'col-sm-12': true, 'has-error': errors.has('cargo') }">
                        ...
                        <div class="col-sm-7">
                            ...
                            <input v-show="parseInt(formData.docu_origen) !== 1" ref="cargo" v-model="formData.docu_cargo" v-validate="'required|max:70'" type="text" name="cargo" class="form-control" @change="formData.docu_cargo = formData.docu_cargo.toUpperCase()" />
                    ...
                    <div v-show="parseInt(formData.docu_origen) !== 1" :class="{ 'col-sm-12': true, 'has-error': errors.has('celular') }">
                        <label class="col-sm-2 control-label">Celular</label>
                        <div class="col-sm-7">
                            <input ref="celular" v-model="formData.docu_telef" v-validate="'numeric|digits:9'" type="text" class="form-control" name="celular" />
                            <span v-show="errors.has('celular')" class="help-block">{{ errors.first('celular') }}</span>
                        </div>
                    </div>
                    <div v-show="parseInt(formData.docu_origen) !== 1" :class="{ 'col-sm-12': true, 'has-error': errors.has('correo') }">
                        <label class="col-sm-2 control-label">Correo electrónico</label>
                        <div class="col-sm-7">
                            <input ref="correo" v-model="formData.docu_emailorigen" v-validate="'email'" type="email" class="form-control" name="correo" />
                            <span v-show="errors.has('correo')" class="help-block">{{ errors.first('correo') }}</span>
                        </div>
                    </div>
                    <div v-show="parseInt(formData.docu_origen) !== 1" :class="{ 'col-sm-12': true, 'has-error': errors.has('direccion') }">
                        <label class="col-sm-2 control-label">Dirección</label>
                        <div class="col-sm-7">
                            <input ref="direccion" v-model="formData.docu_domic" v-validate="'max:150'" type="text" name="direccion" class="form-control" @change="formData.docu_domic = formData.docu_domic.toUpperCase()" />
            ...
            <div class="panel-body">
                <div class="form-group" style="width: 100%;">
                    <div v-if="!plantilla" class="col-sm-12">
                        ...
                        <div class="col-sm-4">
                            <input id="docu_fecha_doc" v-model="formData.docu_fecha_doc" type="date" class="form-control" name="docu_fecha_doc" />
                        </div>
                    </div>
                    <div v-if="!(viewer || (edit && parseInt(formData.docu_origen) === 1))" :class="{ 'col-sm-12': true, 'has-error': errors.has('tipo de documento') }">
                        <label class="col-sm-2 control-label">Tipo de Documento</label>
                        <div class="col-sm-7">
                            <div v-if="edit && parseInt(formData.docu_origen) !== 2">
                                {{ getTipoDocumentoNombre(formData.docu_idtipodocumento) }}
                            </div>
                            <el-select v-if="!(edit && parseInt(formData.docu_origen) !== 2)" 
                            ref="tipo de documento" 
                            v-model="formData.docu_idtipodocumento"  
                            size='small' @change="chngTipoDocumento()" 
                            style="width: 100%;" 
                            placeholder="Tipo documento" 
                            filterable>
                                <el-option v-for="(tipoDocumento, indexTipo) in getTiposDocumento" 
                                :key="indexTipo" 
                                :label="tipoDocumento.tdoc_descripcion" 
                                :value="tipoDocumento.idtdoc"
                                :selected="tipoDocumento.tdoc_descripcion">
                                </el-option>
                            </el-select>
                            <!-- <select
                ...
                >
                  ...
                  errors.first('tipo de documento')
                    ...
                    <div v-show="!(viewer || (edit && parseInt(formData.docu_origen) === 1))" :class="{ 'col-sm-12': true, 'has-error': errors.has('numero de documento') || errors.has('siglas') }">
                        <label class="col-sm-2 control-label">{{ !plantilla ? 'Número y ' : '' }}Siglas</label>
                        <div v-if="!plantilla" class="col-sm-2">
                            <input ref="numero de documento" v-model="formData.docu_numero_doc" v-validate="'numeric'" type="text" class="form-control text-right" :readonly="existCorrelativo" name="numero de documento" />
                            <span v-show="errors.has('numero de documento')" class="help-block">{{
                  errors.first('numero de documento')
                }}</span>
                        </div>
                        <div class="col-sm-8">
                            <input ref="siglas" v-model="formData.docu_siglas_doc" v-validate="'max:65'" type="text" name="siglas" class="form-control" :readonly="parseInt(formData.docu_origen) === 1" @change="formData.docu_siglas_doc = formData.docu_siglas_doc.toUpperCase()" />
                    ...
                    <div class="col-sm-12">
                        ...
                        <div class="col-sm-4">
                            ...
                            <select v-else v-model="formData.docu_idrecepcion" class="form-control" @change="chngTipoDocumento()">
                                <option v-for="(recepcion, indexRecep) in recepciones" :key="indexRecep" :value="recepcion.idrecepcion">
                        ...
                        <div id="docu_idprioridad" class="col-sm-4">
                            ...
                            <select v-else v-model="formData.docu_idprioridad" class="form-control">
                                <option v-for="(prioridad, indexPriori) in prioridades" :key="indexPriori" :value="prioridad.idprioridad">
                    ...
                    <div :class="{ 'col-sm-12': true, 'has-error': errors.has('folios') }">
                        ...
                        <div class="col-sm-4">
                            <input ref="folios" v-model="formData.docu_folios" v-validate="'required'" type="number" class="form-control" name="folios" />
                    ...
                    <div :class="{ 'col-sm-12': true, 'has-error': errors.has('asunto') }">
                        ...
                        <div class="col-sm-10">
                            <textarea ref="asunto" v-model="formData.docu_asunto" v-validate="'required|max:750'" name="asunto" class="form-control" @change="
                    formData.docu_asunto = formData.docu_asunto.toUpperCase()
                    markForGenerarDocumento()"></textarea>
              ...
              " class="col-sm-12">
                        ...
                        <div class="col-sm-2" style="position:relative;">
                            <button class="btn btn-sm btn-success" type="button">
                                ...
                                <input v-if="uploadReady" ref="fileInput" type="file" style="position:absolute;z-index:2;top:0;left:0;filter: alpha(opacity=0);-ms-filter:&quot;progid:DXImageTransform.Microsoft.Alpha(Opacity=0)&quot;;opacity:0;background-color:transparent;color:transparent;" size="40" name="docu_archivo" accept=".pdf" @change="submitFile($event, true)" />
                            </button>
                        </div>
                        <div class="col-sm-8">
                                ...
                                <tbody class="success">
                                    <tr v-for="(docu, index) in formData.docu_archivo.filter(d => d.file_mostrar)" :key="index">
                                        ...
                                        <td>{{ docu.file_name }}<br />
                                            <span v-if="docu.edit===true" class="badge badge-warning">El documento será generado al Guardar</span></td>
                                        <td>{{ getFormatBytes(docu.file_size) }}</td>
                                        <td>{{ (docu.file_principal != null)?((docu.file_principal)?'Principal':'Anexo'):'' }}</td>
                                        <td>
                                            <div>
                                                <button v-if="docu.file_generado && $route.name!=='DocuGeneradoEdit2'" class="btn btn-xs btn-success" title="Editar" @click.prevent="editarFile(docu)">
                                                    <span class="glyphicon glyphicon-pencil"></span>
                                                </button>
                                                <button v-if="showButtonDelete(docu)" class="btn btn-xs btn-danger" title="Eliminar" @click.prevent="ocultarFile(docu.file_url)">
                                                    <span class="glyphicon glyphicon-trash"></span>
                                                </button>
                                                <button v-if="canSuper" class="btn btn-xs btn-warning" title="Editar" @click.prevent="openRemplaceFile(docu)">remplazar
                                                </button>
                                            </div>
                                        </td>
                                    </tr>
        ...
        <docu-derivar :derivaciones="derivaciones" :derivaciones2="derivaciones2" :viewer="viewer" :generado="archivoPrincipalGenerado!==undefined||viewer||plantilla" @derivar="markForGenerarDocumento" />
    ...
    <docu-invoker ref="foo" :iframe="iframe" :route-invoker-get="routeInvokerGet" :route-invoker-post="routeInvokerPost" :rutaurl="rutaurl" />
    <div id="editar" class="modal" role="dialog">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-body" style="padding: 0;">
                    <iframe ref="editor" title="Inline Frame Example" width="100%" height="770" scrolling="auto" src="/tramite/enproceso/editor">
                    </iframe>
                </div>
            </div>
        </div>
                ...
                <div class="modal-body">
                    <div class="form-group">
                        <div class="col-sm-2" style="position:relative;">
                            <button class="btn btn-sm btn-warning" type="button">
                                Arhivo
                                <input ref="file_remplace" type="file" style="position:absolute;z-index:2;top:0;left:0;filter: alpha(opacity=0);-ms-filter:&quot;progid:DXImageTransform.Microsoft.Alpha(Opacity=0)&quot;;opacity:0;background-color:transparent;color:transparent;" size="40" name="file_remplace" accept=".JPG,.PNG,.pdf,.xlsx,.docx" @change="fileRemplace($event)" />
                            </button> &nbsp;
                            <span id="upload-file-info" class="label label-info"></span>
                        </div>
...
import {
    Documento
} from "@/js/store/models/documento"
...
export default {
    ...
    mounted() {
        document.addEventListener('saveIframe', this.guardarDocumentoGenerado, {
            passive: true
        })
        document.addEventListener('closeIframe', this.cerrarModal, {
            passive: true
        })
    ...
    methods: {
        ...
        generarDocumento() {
            let html = '<div>&nbsp;</div><div>&nbsp;</div>' +
                '<div style="text-align:center;">Documento firmado digitalmente<br>' +
                '<strong>' + this.formData.docu_firma +
                '</strong>' + '<br>' + this.formData.docu_cargo + '</div><br>' +
                '<div style="font-size:6px;"><strong>CC:</strong><br>ARCHIVO</div>'
            ...
            let event = new CustomEvent('resetSaving', {
                detail: null
            })
        ...
        mostrarIframe(file) {
            ...
            let reset = new CustomEvent('resetSaving', {
                detail: null
            })
            let put = new CustomEvent('putHTML', {
                detail: file.file_html
            })
            ...
            if (confirm('Esta seguro de cambiar el archivo?, la acción sera irreversible!')) {
                if (e.length > 0) {
                    return new Promise((resolve, reject) => {
                        ...
                        return axios.post('/tramite/documento/remplaceFile', f, {
                                headers: {
